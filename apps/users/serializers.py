...
from .models import UserProfile, VerifyCode,WeiboAndUser
...
class SmsSerializer(serializers.ModelSerializer):
    ...
    def validate_mobile(self, mobile):
        ...
        if User.objects.filter(mobile=mobile).count():  # 手机是否注册
            raise serializers.ValidationError('用户已存在')

        if not re.match(settings.REGEX_MOBILE, mobile):  # 验证手机号吗是否合法
                                                          # REGEX_MOBILE = r'1[3-8]\d{9}'
            raise serializers.ValidationError('手机好吗不合法')

        one_mintes_ago = datetime.now() - timedelta(minutes=1)  # 验证码发送频率
